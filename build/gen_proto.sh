...
function tfx::copy_proto_stubs() {
  ...
  set -e
  find ${src_dir} -name "*_pb2.py" -exec cp -f {} ${dst_dir}/{} \;
  find ${src_dir} -name "*_pb2.pyi" -exec cp -f {} ${dst_dir}/{} \;
  find ${src_dir} -name "*_pb2_grpc.py" -exec cp -f {} ${dst_dir}/{} \;
  find ${src_dir} -name "*_pb2_grpc.pyi" -exec cp -f {} ${dst_dir}/{} \;
}
